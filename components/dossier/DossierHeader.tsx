 ...
 * Audience links are destinations within one canonical record; they never swap medical copy.
 */
export function DossierHeader({ dossier }: DossierHeaderProps) {
  const hasClinicalView = dossier.keyOutcomes.length > 0 || dossier.studies.length > 0
  const hasBiotechView =
  ...
  return (
    <header className="min-w-0 border-b border-black/[0.08] pb-6 pt-2 sm:pb-7 sm:pt-6">
      ...
      <div className="mt-6 grid min-w-0 gap-4 border-t border-black/[0.08] pt-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
        ...
        <div className="min-w-0 space-y-2 lg:text-right">
          <nav aria-label="Choose a reading depth">
            <ul className="inline-flex max-w-full flex-wrap items-center gap-1 rounded-2xl border border-black/[0.08] bg-white p-1 text-xs font-semibold shadow-[0_1px_4px_rgba(0,0,0,0.025)]">
              <li>
                <a
                  href="#what-it-is"
                  className="inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-blue-50 px-3 text-[#0A66D8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0A66D8]"
                >
          ...
          <p
            id="approval-status"
            className="inline-flex min-h-6 items-center gap-1.5 text-xs leading-5 text-[#6E6E73]"
          >
