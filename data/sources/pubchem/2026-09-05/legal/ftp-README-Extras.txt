...
CID-MeSH:
    ...
    This file contains associations between CID and MeSH terms.
...
MeSH-Pharm:
    ...
    the PubChem Compound database; it is thus only a
    subset of all MeSH terms. Along with the aforementioned
    CID-MeSH files, this allows one to map CID to
...
CID-Identifiers.gz:
    ...
    This is a listing of all CIDs with their third-party database
    identifiers. It is a gzipped text file with CID, tab, identifier,
    tab, identifier type on each line.
...
PubChem-Reference-Collection.tsv.gz:
	
    This is a dump of PubChem Reference Collection Records. It contains
    multiple columns separated by tab, e.g. SID, RefChemID, PreferredName, 
    Synonyms, PreferredCID, OtherCIDs, PreferredInChIKey, OtherInChIKeys, 
    Related, CreateDate, ModifyDate, etc. If there are multiple records in 
    a column, "|" is used to separated them. The Related column contains a 
    list of related records and their relationships in the format of 
    "relation:PreferredName". For records that don't have a discrete 
    structure, the PreferredCID is 0.

Drug-Names.tsv.gz:

    This file contains drug synonyms, drug names, PubChem CIDs and their 
    corresponding PubChem URLs. A "drug" in this context refers to any records
    identified as an active ingredient in drug labels published on NLM's 
    DailMed Web site.

CID-Biologics.tsv.gz:

    This file contains biologic line notations generated by NextMove's
    Sugar & Splice software. It is a gzipped text file with CID, tab,
    type, tab, and notation on each line. Note that a given CID may
    not have all types of notations. The types are:
