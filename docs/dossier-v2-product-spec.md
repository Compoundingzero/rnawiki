...
2. A short answer with the intended use, clearest recorded result, and main uncertainty. Published
   programme wording remains bound to its reviewed revision; older medicine-wide records use their
   stored plain result fields instead of exposing a dense technical verdict as the first read.
...
5. The original longer wording remains available for an older record, clearly labelled as older
   wording rather than a second conclusion.
...
8. A separate collapsed **More about this medicine** section for useful medicine-wide background
   from the older record. It is a sibling of the evidence disclosure, not part of the proof for the
   selected programme conclusion.
9. Substantive community commentary, when present, remains separate from both reviewed evidence
   and older medicine-wide background. Empty community sections stay hidden.
...
No evidence-bearing copy is defined in a component. A published programme view is assembled from
database rows. Records without a published programme revision keep their older medicine-wide
material visibly separate under labels such as **What the older record says** and **Older record**.
When the reviewed programme record has no answer, the interface says **No reviewed answer yet**; the
application does not create a verdict to fill a gap.
...
Older medicine-wide audit notes are not silently promoted into this causal chain. When no reviewed
programme graph exists, populated notes appear as an unconnected **Older evidence notes** list,
without step numbers, connectors, or reviewed scientific-state badges.
...
## Medicine-wide context retained from the older record
...
as older medicine-wide context; none is used to construct or support a programme conclusion.
...
- Common answers are labelled as background, not instructions for starting, stopping, or changing
  treatment.
- Safety and administration wording remains closed by default and is explicitly general older-record
...
- Older evidence notes lead with their plain summary. Their stored measured metric, exact technical
...
published claims, and published nodes are internally consistent. Otherwise, older medicine-wide
material remains visibly labelled **Older medicine-wide summary** or **Older record**, and missing
reviewed programme answers say **No reviewed answer yet**. Existing content is never silently
promoted into a reviewed programme verdict.
