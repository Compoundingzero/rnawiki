...
function r5(items, publish, refuse, warn) {
  ...
  const cov = { checked: ENGINE.covered(list).length, of: list.length };
...
function validate({ spec, base_pid, base_rcid, publish }) {
  ...
  const says = !coverage || coverage.of === 0
    ? 'No compounds in this protocol, so there was nothing to cross-check.'
    : coverage.of === 1
      ? 'Only one compound here, so there is no pairing to check.'
      : coverage.checked < 2
        ? `Not enough to check — RNAwiki holds interaction pharmacology for ${coverage.checked} of these ${coverage.of}. Nothing flagged against the rest is an absence of data, not a clearance.`
