...
const PATTERNS = [
  ...
  { id: 'harness/unlock/empower/elevate', re: /\b(harness(es|ed|ing)?|unlock(s|ed|ing)?|empower(s|ed|ing)?|elevat(e|es|ed|ing))\b/gi },
  { id: 'seamless/robust/cutting-edge', re: /\b(seamless(ly)?|robust|cutting[- ]edge|state[- ]of[- ]the[- ]art|game[- ]chang\w+)\b/gi },
  { id: 'navigate (figurative)', re: /\bnavigat(e|es|ed|ing)\s+(the|this|a)\s+\w*(landscape|complexity|world|maze)/gi },
  { id: 'underscore/highlight the importance', re: /\b(underscor\w+|highlight\w*)\s+(the\s+)?(importance|need|significance)/gi },
  ...
  { id: 'at its core / at the heart of', re: /\b(at its core|at the heart of|when it comes to)\b/gi },
  { id: 'plays a (vital|key) role', re: /\bplays? an? (vital|key|crucial|pivotal|important|significant) role\b/gi },
  { id: 'it is important/worth noting', re: /\bit(?:'s| is) (important|worth) (to note|noting)\b/gi },
  ...
  { id: 'revolutionary/groundbreaking', re: /\b(revolutionar\w+|ground[- ]?breaking|transformative|paradigm[- ]shift\w*)\b/gi },
...
for (const file of files) {
  ...
  for (const { id, re } of PATTERNS) {
    ...
    while ((m = re.exec(prose)) !== null) {
      ...
      hits.get(id).push({ file: file.replace(/^.*\/(?=(components|app|lib|scripts))/, ''), match: m[0], context })
    }
  }
}
