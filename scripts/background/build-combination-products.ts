...
import { PRODUCT_REGISTRY, SUBSTANCE_REGISTRY } from '../seed-data/background/registries'
...
/**
 * Published labels a product needs before it gets a page.
 *
 * One. A product with a single published label is still a product somebody was handed, and a reader
 * searching its name deserves the label rather than an empty result. The earlier floor of three
 * excluded roughly 1,600 real products to keep the set tidy, which served the dataset rather than
 * the reader.
 */
const MIN_LABELS_FOR_A_PAGE = 1
...
async function build(
  ...
  for (const [productKey, product] of wanted) {
    ...
    const ingredients: RecordedIngredient[] = product.ingredients.map((entry) => {
      ...
      // Recorded uses count as data. For a homeopathic preparation, a botanical extract or a
      // mineral it is usually the only thing any source states, and calling those ingredients
      // undocumented while their label plainly says what they are for would report a gap that is
      // not there.
  ...
  const outPath = join(process.cwd(), 'data', 'registries', 'combination-products.json')
