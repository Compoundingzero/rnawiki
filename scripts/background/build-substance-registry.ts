...
async function build(
  ...
  const outPath = join(
    ...
    'seed-data',
    'background',
    'substance-registry.generated.ts',
  )
...
function serialize(registry: Record<string, RegisteredSubstance>): string {
  const entries = Object.keys(registry)
    ...
    .join('\n')
  return `// Generated by scripts/background/build-substance-registry.ts — do not edit by hand.
//
// What is known about each active ingredient, keyed by its RxNorm ingredient concept and read only
// from labels about that substance alone. A mechanism belongs to a substance rather than to a
// carton, so recording it once here means every product containing the ingredient shows the same
// fact and a correction reaches all of them.
...
export const SUBSTANCE_REGISTRY: Record<string, RegisteredSubstance> = {
${entries}
}
`
}
