...
export const ENRICHED_BATCH_11_DOSSIERS: SeedDossier[] = [
  ...
  // ---------------------------------------------------------------------------------------------
  {
    ...
    substitutes: {
      summary:
        'The realistic alternatives split into three groups: the other proton pump inhibitors, which differ from omeprazole mainly in how they are metabolised rather than in what they do; the H2 blockers, which are measurably weaker and stop working within days as tolerance develops; and sucralfate, which is weaker still. Nothing sold as a food raises gastric pH for long enough to heal an erosion, and the comparison for reflux is with the non-drug measures that have their own randomised evidence — weight loss and raising the head of the bed — rather than with a supplement.',
      conventionalRx: [
        {
    ...
    commonQuestions: [
      {
        ...
        a: 'Because the active substance cannot be patented and the delivery system can. 5-aminosalicylic acid has been a known chemical since the nineteenth century. Taken without a delayed or targeted formulation, it is absorbed in the small intestine and does not reach the colon where the disease is. Every product on the market is a different engineered answer to that: a coat that dissolves only above a particular pH, microgranules that release slowly over hours, a matrix that disperses the drug through the colon, an enema or a suppository. Those delivery systems are patentable and are what the brand names refer to. It is also why the formulation matters clinically — a suppository treats the rectum, an enema reaches about as far as the splenic flexure, and extensive disease needs an oral product.',
      },
      {
    ...
    commonQuestions: [
      {
        ...
        a: 'The drug’s chemistry supports long-term safety, but large long-term trials are missing. The label on the box specifies up to seven days. In practice children with functional constipation take it daily for months or years, and every major paediatric guideline recommends exactly that, because stopping early is how the problem comes back. The polymer is far too large to be absorbed, there is no transporter for it, and it is not metabolised, so essentially the entire dose leaves in stool having done nothing except carry water. The Cochrane reviewers said explicitly that research into long-term use is needed, and that was in 2016. The largest randomised study over a full year included 97 children and found no drug-related serious adverse events, which is reassuring but cannot rule out an uncommon one.',
        auditNote:
