#!/usr/bin/env python
"""Gate 1b measurement v3 (Phase 2d): the same derivation as gate1b_v2.py — matched 803 and 324
draws, size-matched folds, the bucket threshold rule — re-run over the page text rendered after the
Phase 2d fixes (evidence age as a value inside the human-data block, the three CLINICAL templates,
the standing paragraph-2 fallbacks removed). Definitions, sets and seeds are unchanged, so v1, v2
and v3 are comparable line for line.

Stages (run in order):

    build   read data/corpus-20k/render/text/batch-*.ndjson + questions/, write the harness inputs
            under data/corpus-20k/render/v3/ and the page metadata under data/corpus-20k/gate1b/
...
OUT_PAGES = ROOT / "data/corpus-20k/render/v3"
...
def stage_build() -> dict:
    ...
    (OUT_GATE / "indexed-keys-v3.txt").write_text("\n".join(indexed) + "\n", encoding="utf-8")
    (OUT_GATE / "page-meta-v3.json").write_text(json.dumps(meta), encoding="utf-8")
...
def stage_buckets() -> dict:
    rows = json.loads((OUT_GATE / "matched-per-page-v3.json").read_text(encoding="utf-8"))
...
def stage_folds() -> dict:
    ...
    (OUT_GATE / "matched-per-page-v3.json").write_text(json.dumps(rows), encoding="utf-8")
    return analyse_matched(rows, folds, owners)


def analyse_matched(rows: dict[str, dict], folds, owners) -> dict:
    n = len(rows)
    meta = json.loads((OUT_GATE / "page-meta-v3.json").read_text(encoding="utf-8"))
    ...
    (OUT_GATE / "buckets-matched-v3.json").write_text(json.dumps(buckets, indent=1), encoding="utf-8")
    (OUT_GATE / "threshold-v3.json").write_text(json.dumps(thr, indent=1), encoding="utf-8")
    ...
    (OUT_GATE / "matched-pooled-v3.json").write_text(json.dumps(pooled, indent=1), encoding="utf-8")
...
def stage_shared() -> dict:
    ...
    (OUT_GATE / "shared-content-v3.json").write_text(json.dumps(out, indent=1), encoding="utf-8")
...
RUNS = [
    ("rendered-803", "pages-indexed-803.ndjson", ["--exhaustive"],
     "v3: rendered dossier text, seeded 803 draw from the indexed set"),
    ("rendered-324", "pages-indexed-324.ndjson", ["--exhaustive"],
     "v3: rendered dossier text, seeded 324 draw from the indexed set"),
    ("indexed", "pages-indexed.ndjson", [],
     "v3: rendered dossier text, Tier 1 + Tier 2 with >= 1 question"),
    ("indexed-prose", "pages-indexed-prose.ndjson", [],
     "v3 sensitivity: prose only, declared-markup rows removed"),
    ("full-corpus", "pages-all.ndjson", [],
     "v3: rendered dossier text, all pages including Tier 3"),
]
...
def stage_validate() -> dict:
    ...
    cmd = [
        ...
        "--out", str(target),
        "--corpus-label", "rendered-indexed-803 (v3)",
    ]
    print("$ " + " ".join(cmd), flush=True)
    subprocess.run(cmd, check=True, cwd=str(ROOT))
...
# ------------------------------------------------------------------------------------------
# summary — assemble summary-v3.json and report-v3.md
...
def stage_summary() -> dict:
    ...
    runs = {name: _read(OUT_GATE / "runs-v3" / name / "summary.json") for name, *_ in RUNS}
    pooled = _read(OUT_GATE / "matched-pooled-v3.json")
    thr = _read(OUT_GATE / "threshold-v3.json")
    buckets = _read(OUT_GATE / "buckets-matched-v3.json")
    shared = _read(OUT_GATE / "shared-content-v3.json")
    validation = _read(OUT_GATE / "validation-v3" / "validation.json")
    ...
    rows = _read(OUT_GATE / "matched-per-page-v3.json")
    meta = _read(OUT_GATE / "page-meta-v3.json")

    # like-for-like: the same 4,558 pages v2 measured, scored on the v3 render
    ...
    like = {
        "note": "The v3 render scored on exactly the 4,558 pages v2 measured, so the two numbers "
    ...
    summary = {
        ...
        "method": {
            ...
            "driver": "scripts/corpus-20k/overlap/gate1b_v3.py (build | folds | buckets | shared | runs | validate | summary)",
            "sets": v2["method"]["sets"],
        ...
        "renderer": {
            ...
            "out": "data/corpus-20k/render/text/batch-NNNN.ndjson (step page-text-v3, 29 batches)",
            "pages": render["pages"],
    ...
    summary["diffVsV1AndV2"] = {
        "note": "v1 = summary.json (Phase 2b MEASURE); v2 = summary-v2.json (Phase 2c re-measure); "
                "v3 = this run. The v2 column's 'none clears' rows are written as null.",
        "rows": [{"measure": m, "v1": a, "v2": b, "v3": c} for m, a, b, c, _ in diff_rows],
    }

    (OUT_GATE / "summary-v3.json").write_text(json.dumps(summary, indent=1, ensure_ascii=False) + "\n", encoding="utf-8")
...
def write_report(summary, diff_rows, runs, buckets, thr, pooled, standing, like, validation, metrics, render, shared) -> None:
    ...
    a("# Gate 1b — measurement v3 (Phase 2d)")
    a("")
    ...
    a("")
    a("**What changed in the pages between v2 and v3.** Derived seed 15 (evidence age) no longer renders")
    a("as a block: its year and record are one sentence inside the human-data block's paragraph 2, and its")
    a("revealed row is the NCT and the completion date, with no `asOf` and no `yearsSince` label. Three")
    a("CLINICAL templates were added (label indication, registers-only, trial history), which gave a")
    ap = f"question to {like['newlyIndexed']['pages']:,} CLINICAL pages that fired none before. Four paragraph-2"
    a(ap)
    a("fallbacks that were the same sentence on many pages were removed or given the page's own values.")
    a("The suppression classes were re-run over the pass-2 corpus with S1 narrowed to ATC N01A.")
    a("")
    if clears:
    ...
    a("")
    a("v3 indexes 5,883 candidates against v2's 4,558, because the three CLINICAL templates give a first")
    a("question to thin pages that previously had none. A pooled median over a different set of pages is")
    a("not a before-and-after. Scoring the v3 render on exactly the 4,558 pages v2 measured:")
    a("")
    ...
    a(f"| the same pages, v3 render | {like['carriedFromV2']['pages']:,} | {like['carriedFromV2']['positionalMedian']:.4f} |")
    ...
    a("## Harness fit")
    a("")
    a(f"Re-validated at the 803 draw of the v3 render, as R3 requires before any corpus-scale figure is")
    ...
    a("")
    a("| Measure | v1 (Phase 2b) | v2 (Phase 2c) | v3 (this run) | v2 → v3 |")
    a("| --- | ---: | ---: | ---: | ---: |")
    ...
    a("## Files")
    a("")
    a("- `data/corpus-20k/gate1b/summary-v3.json` — every figure here, machine-readable")
    a("- `data/corpus-20k/gate1b/runs-v3/*/` — one directory per measured set")
    a("- `data/corpus-20k/gate1b/matched-per-page-v3.json`, `matched-pooled-v3.json`,")
    a("  `buckets-matched-v3.json`, `threshold-v3.json`, `page-meta-v3.json`, `indexed-keys-v3.txt`")
    a("- `data/corpus-20k/gate1b/shared-content-v3.json` — the 5-gram attribution")
    a("- `data/corpus-20k/gate1b/validation-v3/validation.json` — the harness fit run")
    a("- `data/corpus-20k/render/text/standing-sentences.json` — the standing-sentence audit")
    a("- `data/corpus-20k/render/v3/pages-*.ndjson` — harness inputs built from the v3 render")
    a("- `scripts/corpus-20k/overlap/gate1b_v3.py` — the driver")
    a("")
    (OUT_GATE / "report-v3.md").write_text("\n".join(L), encoding="utf-8")
